...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, TranslateModule, ReactiveFormsModule, ToastModule, InputNumberModule],
...
export class ConfiguracoesGlobaisComponent implements OnInit {
  ...
  configForm = this.fb.group({
    ...
    rfTempoRftdt: [1.10, Validators.required],
    ...
    this.configService.atualizar(payload).subscribe({
      next: () => {
        ...
        this.messageService.add({ 
            ...
            detail: 'Entradas Padrão (Classe D/O) atualizadas com sucesso!' 
        });
        this.carregarConfiguracoes();
      },
      error: () => {
        ...
        this.messageService.add({ 
            severity: 'error', 
            summary: 'Erro', 
            detail: 'Erro ao salvar entradas padrão.' 
        });
      }
    });
  }
